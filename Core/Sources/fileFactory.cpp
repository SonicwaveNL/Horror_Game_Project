    ...
    for (auto & row : matrix) {
        for (auto & item : row) {
            ...
            if (soort == objectType::Abstract) {
                ...
                auto pointer =
                    std::make_unique<Wall>(item.getPosition(), returnVector);
            } else if (soort == objectType::Switch) {
                auto pointer =
                    std::make_unique<Switch>(item.getPosition(), returnVector);
            } else if (soort == objectType::Door) {
                auto pointer =
                    std::make_unique<Door>(item.getPosition(), returnVector);
            } else if (soort == objectType::Player) {
                auto pointer =
                    std::make_unique<Player>(item.getPosition(), returnvector);
            } else if (soort == objectType::Monster) {
                auto pointer =
                    std::make_unique<Monster>(item.getPosition(), returnvector);
            }
        }
    }
