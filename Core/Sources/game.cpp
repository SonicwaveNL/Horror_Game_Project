    ...
    for (size_t j = 0; j < amountOfRow; j++) {
      ...
      if (i == 0 || i == (amountOfColumn - 1) || j == 0 ||
        ...
        shapeMatrix[posX][posY].setCellType(objectType::Wall,
                                            &gameTextures[objectType::Wall][0]);
      }
      y += PIXEL16;
    }
    x += PIXEL16;
...
void Game::draw(std::set<std::shared_ptr<IObject>> &objects) {
  for (auto &object : objects) {
    object->draw(window);
  }
}
  ...
  while (window.isOpen()) {
    ...
    switch (currentState) {
      ...
      case gameState::Play: {
        ...
        // sound
        if (std::fabs(monster->getPosition().x - player->getPosition().x) <=
                50 ||
            std::fabs(monster->getPosition().y - player->getPosition().y) <=
                50) {
          sound.setTense();
        } else {
          sound.setNormal();
