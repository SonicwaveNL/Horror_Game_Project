...
class IObject {
  ...
  public:
    ...
    IObject(sf::Texture & texture, sf::Vector2f position,
            std::vector<std::shared_ptr<IObject>> & objects,
            std::string type = "IObject", sf::Color color = sf::Color::White)
        : texture(texture), position(position), objects(objects), type(type),
          color(color)
    ...
    ///@param color
    /*a SFML sf::Color, this is the color of the texture.*/
    IObject(sf::Vector2f position, sf::Vector2f size,
            std::vector<std::shared_ptr<IObject>> & objects,
            std::string type = "IObject", sf::Color color = sf::Color::White)
        : color(color), base(size), position(position), objects(objects),
          type(type)
